...
After running the experiments, an obvious bottleneck could be found. The current brute-force equivalence query implementation is inefficient, as the theoretical evaluation suggested. Figure \ref{fig:stateruntime} and \ref{fig:inputruntime} shows the inefficiency observed. In order to show (and compare) my implementation without this bottleneck, Fig. \ref{fig:inputruntime1} shows the exact same run done with a near linear-time equivalence oracle implemented in LearnLib.
